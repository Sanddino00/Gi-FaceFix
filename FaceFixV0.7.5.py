...
def smart_replace(file_path, make_backup=True, preview=True, apply_changes=False):
    """Scan and optionally replace lines in the file."""
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            content = f.read()
            
        # Skip files containing DISABLED
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_file = {executor.submit(smart_replace, ini_file, make_backup, False, True): ini_file for ini_file in files_to_modify}
